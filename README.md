...
Agent-Kryptos is a secure, zero-knowledge secret provisioning system designed to enable humans to securely inject sensitive credentials (like API keys, passwords, or tokens) directly to AI Agents.
...
Agent-Kryptos solves this by intercepting requests for secrets at the Gateway level and generating a secure, single-use, out-of-band link for the user. The user encrypts the secret in their browser, and only the agent's constrained execution environment can decrypt and hold it in memory, using it to fulfill API calls entirely transparently.
...
4. **Gateway Middleware:** Intercepts LLM tool calls, replaces them with secure out-of-band links, and enforces outbound URL filtering to prevent phishing or data exfiltration.
