...
| TM-007 | Remote attacker or concurrent guest | Valid public offer token and payment capability | Submit duplicate guest payment requests concurrently to exploit GET-then-INSERT TOCTOU in `verifyAndSettleGuestPayment` | Both requests pass existence check, both proceed to call `provider.verifyPayment` and `provider.settlePayment`, causing duplicate settlement on the facilitator | Payment integrity, billing state, facilitator trust | `ON CONFLICT DO NOTHING` on insert prevents duplicate DB rows; x402 facilitator may reject duplicate settlements | The non-inserting request still proceeds to verify/settle; no atomic gate between existence check and provider calls | Use a DB transaction with conditional `INSERT ... RETURNING *` or advisory lock to ensure only one request proceeds to the provider | Monitor for duplicate settlement attempts per intent_id; alert on facilitator rejection spikes | Medium | Medium | high |
...
- TM-007 (guest payment TOCTOU) added to cover race-condition-based payment fraud, cross-referenced with [H-6 in Security Audit v2](file:///home/hvo/Projects/blindpass/docs/security/Security%20Audit%20v2.md).
